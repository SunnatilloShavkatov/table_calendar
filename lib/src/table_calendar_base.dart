...
class _TableCalendarBaseState extends State<TableCalendarBase> {
  ...
  void initState() {
    ...
    final int rowCount = _getRowCount(widget.calendarFormat, _focusedDay);
    _pageHeight = ValueNotifier(_getPageHeight(rowCount));

    final int initialPage = _calculateFocusedPage(
  ...
  void didUpdateWidget(TableCalendarBase oldWidget) {
    ...
    if (_focusedDay != widget.focusedDay ||
      ...
      final bool shouldAnimate = _focusedDay != widget.focusedDay;
  ...
  void _updatePage({bool shouldAnimate = false}) {
    ...
    final int endIndex = _calculateFocusedPage(
      widget.calendarFormat,
      widget.firstDay,
      widget.lastDay,
    );
  ...
  double _getPageHeight(int rowCount) {
    final double tablePaddingHeight = widget.tablePadding?.vertical ?? 0.0;
    final double dowHeight = widget.dowVisible ? widget.dowHeight! : 0.0;
  ...
  int _getDaysBefore(DateTime firstDay) =>
      (firstDay.weekday + 7 - getWeekdayNumber(widget.startingDayOfWeek)) % 7;
